...
(function() {
  $(function() {
    ...
    $('input#field1016').on('change', function() {
      if (/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/.test(field[0].value || field.value === '')) {
        ($('button#sdhButton'))[0].innerHTML = '<img src="' + chrome.runtime.getURL("img/icon16.png") + '" alt="">Перейти';
      } else {
        if (field[0].value !== '') {
          ($('button#sdhButton'))[0].innerHTML = '<img src="' + chrome.runtime.getURL("img/icon16.png") + '" alt="">Ошибка';
        } else {
          ($('button#sdhButton'))[0].innerHTML = '<img src="' + chrome.runtime.getURL("img/icon16.png") + '" alt="">Перейти';
        }
      }
